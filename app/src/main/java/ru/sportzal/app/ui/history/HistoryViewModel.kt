...
data class HistoryUiState(val workouts: List<HistoryWorkout> = emptyList(), val details: HistoryWorkoutDetails? = null,
...
class HistoryViewModel(private val repository: SportzalRepository) {
    ...
    }.onFailure { mutableState.value = mutableState.value.copy(loading = false, error = "Не удалось загрузить историю") }
    ...
    suspend fun saveNote(text: String): Boolean = commit { repository.updateWorkoutNotes(requireNotNull(state.value.details).runtime.workoutId, text) }
    suspend fun deleteSet(id: String): Boolean = commit { repository.deleteSet(id) }
    suspend fun editSet(old: ru.sportzal.app.data.db.SetResultEntity, weight: Double, reps: Int, rir: Int?, note: String?): Boolean = commit {
        repository.editSet(EditSetCommand(old.setResultId, weight, reps, rir, java.time.Instant.now().toString(),
            StrictJson.decodeFromString(old.deviationsJson), note, old.equipmentIdActual, old.equipmentNameActual,
            old.setupActual, old.loadBasisActual, old.sideActual))
    }
    private suspend fun commit(block: suspend () -> Unit): Boolean {
        ...
        return runCatching { block(); reloadDetails(id); load() }.fold({ true }, {
            mutableState.value = mutableState.value.copy(saving = false, error = it.message ?: "Не удалось сохранить"); false
        })
    }
    ...
    }.onFailure { mutableState.value = mutableState.value.copy(loading = false, saving = false, error = "Не удалось загрузить историю") }
}
